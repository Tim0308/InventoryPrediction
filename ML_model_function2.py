...
def top_sales_hospital_prediction_stacking(sales_table, horizon_days):
    ...
    if sales_table is None or getattr(sales_table, 'empty', True):
        print("Skipping: empty sales_table passed to stacking predictor.")
        return None
    ...
    if n_samples < MIN_SAMPLES:
        ...
        return None
    if n_samples >= 3:
        # Adaptive number of splits to avoid errors on very small datasets
        num_splits = min(5, n_samples - 1)
        ...
        raise ValueError("Not enough samples to train: got 0 rows after preprocessing.")
    ...
    small_data = n_samples < 3

    # Helpers to fit/predict base learners safely for tiny datasets
    def fit_bases(X_tr, y_tr):
        rf_base.fit(X_tr, y_tr)
        if not small_data:
            try:
                lgbm_base.fit(X_tr, y_tr)
            except Exception:
                # If LightGBM fails on tiny data, fall back to RF-only behavior
                pass

    def predict_bases(X_in):
        rf_pred = rf_base.predict(X_in)
        # Try LightGBM; on failure or small data, duplicate RF predictions
        if not small_data:
            try:
                lgbm_pred = lgbm_base.predict(X_in)
                return np.column_stack([rf_pred, lgbm_pred])
            except Exception:
                return np.column_stack([rf_pred, rf_pred])
        return np.column_stack([rf_pred, rf_pred])
    ...
    if n_samples >= 3:
        ...
        for tr_idx, val_idx in tscv.split(X, y):
            ...
            # Fit base models for this fold and predict
            ...
            try:
                ...
                # Duplicate RF predictions if LightGBM fails in a fold
                oof_preds[val_idx, 1] = rf_base_fold.predict(X_val)

            oof_preds[val_idx, 0] = rf_base_fold.predict(X_val)

        # Build meta training set from rows with OOF predictions
        valid_mask_oof = ~np.isnan(oof_preds).any(axis=1)
        X_meta_train = oof_preds[valid_mask_oof]
        y_meta_train = y.iloc[valid_mask_oof]
    else:
        # With tiny data, fit on the holdout train and use its predictions for meta training
        fit_bases(X_tr_hold, y_tr_hold)
        base_pred_tr = predict_bases(X_tr_hold)
        X_meta_train = base_pred_tr
        y_meta_train = y_tr_hold
    ...
    fit_bases(X_tr_hold, y_tr_hold)
    base_pred_test = predict_bases(X_te_hold)
    ...
    # Iterative multi-step forecast for next horizon invoices, using previous-invoice features
    ...
    for _ in range(horizon):
        ...
        base_last = predict_bases(feat_prev)
    ...
    # Report first step (backward compatibility) and the horizon outlook
    ...
    print("Next forecasted invoice dates:")
